...
# Group by and summarize
grouped = df_survival.groupby(['year', 'sex', 'attained_age'])
cancer_suicides = grouped.agg(
    person_years=pd.NamedAgg(column='time', aggfunc=lambda x: round(x.sum())),
    suicide_deaths=pd.NamedAgg(column='suicide', aggfunc='sum')
).reset_index()
...
# Calculate SMRs
cancer_suicides = cancer_suicides.merge(df_suicides, on=['sex'], how='left')
cancer_suicides['expected_suicides'] = (cancer_suicides['person_years'] / 10000) * cancer_suicides['suicides_per_100000']
...
# Dropping NaN values and ensuring data types for merging
cancer_suicides.dropna(inplace=True)
cancer_suicides = cancer_suicides.drop_duplicates()
cancer_suicides['sex'] = cancer_suicides['sex'].astype(str)
cancer_suicides['attained_age'] = cancer_suicides['attained_age'].astype(str)
...
# Calculate SMRs
cancer_suicides = cancer_suicides.merge(df_suicides, on='attained_age', how='left')
cancer_suicides['expected_suicides'] = (cancer_suicides['person_years'] / 10000) * cancer_suicides['suicides_per_100000']
...
# Filter out specific age groups
age_smr = age_smr[(age_smr['attained_age'] != '15-19') & (age_smr['attained_age'] != '20-24')]
...
# Prepare data by ensuring string data types for categorical manipulation
df_survival['sex'] = df_survival['sex'].astype(str)
df_survival['attained_age'] = df_survival['attained_age'].astype(str)
...
# Drop NaN values and ensure distinct records
cancer_suicides.dropna(inplace=True)
cancer_suicides.drop_duplicates(inplace=True)

# Calculate SMRs
cancer_suicides = cancer_suicides.merge(df_suicides, on=['sex', 'attained_age'], how='left')
cancer_suicides['expected_suicides'] = (cancer_suicides['person_years'] / 10000) * cancer_suicides['suicides_per_100000']

# Group by deprivation and summarize
